...
function Loading() {
    return Array(6).fill().map((_, index) => {
        return <div className="px-4 relative card group" key={index}>
            ...
            <div className="w-24 h3 bg-gray-300 mt-3 rounded-full"></div>
            <div className="w-36 h3 bg-gray-300 mt-2 rounded-full"></div>
        </div>
    })
}
    ...
    return (
        <section className="flex flex-col py-16">
            ...
            <div className="overflow-x-hidden px-4" id="carousel">
                ...
                {
                    isLoading ? <div className="flex -mx-4 flex-row relative"><Loading style={{ paddingLeft: refContainer.current?.getBoundingClientRect?.()?.left - 16 || 0, }}></Loading></div> : error ? JSON.stringify(error) : data.data.length === 0 ? "No Product Found" : (
                        <Carousel refContainer={refContainer}>
                            {data.data.map(item => {
                                return <div className="px-4 relative card group" key={item.id}>
                                    <div
